...
function MyBot() {
  ...
  const callingLLM = async () => {
    ...
    if (navigator.deviceMemory < 8) return;
      ...
      <AnimatePresence>
        {activSession && (
          <motion.div
            ...
            className={`rounded-xl  fixed z-20 bottom-16 right-16 bg-gradient-to-r items-start flex justify-center  from-indigo-500 to-purple-500 ${
              !isMobile
                ? "w-80 h-[50%]"
                : "w-[80vw] h-[80%] bottom-[10vh] left-[5vw]"
            }`}
          >
            {navigator.deviceMemory ? (
              ...
              <div>
                <h1 className="text-white text-center text-lg font-bold">
                  Device Memory not supported
                </h1>
                <p className="text-gray-300 text-center text-sm">
                  Please use a device with at least 8GB of RAM.
                </p>
              </div>
            )}
          </motion.div>
